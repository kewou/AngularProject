...
export class ConnexionComponent implements OnInit {
  ...
  loginSubmit() {
    ...
    this.userService.loginUser(this.loginObj).subscribe(
      ...
      (error: HttpErrorResponse) => {
        ...
        if (error.error && error.error.message === "ACCOUNT_NOT_VERIFIED") {
          this.errorMessage =
            "Votre compte n'a pas encore été vérifié. Veuillez vérifier votre boîte mail (y compris le dossier spam) et cliquer sur le lien de validation pour activer votre compte. Si vous n'avez pas reçu l'email, veuillez nous contacter.";
        } else {
  ...
  loginWithGoogle() {
    const redirectUri = encodeURIComponent(
      "http://localhost:4200/beezyApi/login/success"
    );
