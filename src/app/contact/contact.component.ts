...
export class ContactComponent {
  ...
  sendContactMessage() {
    ...
    let sendMessage = {
      ...
      message: this.message.senderMessage,
    };
    this.httpService.post("contact", sendMessage).subscribe(
      (response) => {
        ...
        }, 3000); // 3000ms = 3 secondes, redirection vers la page d'accueil
      },
      (error) => {
        ...
        if (error.status == 500 || error.status == 0) {
          this.errorMessages.push(
            "Erreur survenue lors de l'envoi du mail, nous allons résoudre ce problème dans les plus bref délais"
          );
        }
